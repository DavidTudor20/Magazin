...
public:
    // Constructor
    Produs(const std::string& cod, const std::string& nume, int cant, double p) 
        : cod_de_bare(cod), denumire(nume), cantitate(cant), pret(p) {}

    // Getters
    const std::string getCod() const { return cod_de_bare; }
    const std::string getDenumire() const { return denumire; }
    const int getCantitate() const { return cantitate; }
